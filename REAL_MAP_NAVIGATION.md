...
can rotate through two alternate servers. Public services may throttle requests and do not
...
| `OVERPASS_SECOND_FALLBACK_URL` | `https://maps.mail.ru/osm/tools/overpass/api/interpreter` |
| `OVERPASS_URLS` | Empty; when set, a comma-separated list replaces the three endpoints above |

Set either fallback to an empty string to disable it. Use a single backend worker for
...
Trips up to 12 km load every supported road in the bounding rectangle with a 2 km
margin. Longer motor trips load regional through roads across that rectangle plus
all supported access roads within 2.2 km of each endpoint. Longer walking trips use
an overlapping 2.2 km pedestrian corridor sampled every 3 km. This prevents unrelated
neighbourhood roads from exhausting the graph limit. A detour outside the downloaded
coverage can be missed; optimality is over that graph, not every road worldwide.
...
86 real-map tests. Tests cover new-region UCS reuse, coordinate snapping, one-way
...
15-second execution budget; HTTP connect/read limits are 4/16 seconds, with an
18-second response guard. The UI stops waiting for a trip after 65 seconds and
restores the controls with a retry message.
...
host is revisited once after the alternate attempts, without marking it as failed.
Oversized downloads do not trigger a duplicate download from another server. If every
provider fails, the API can reuse the graph already loaded for the same area and transport
mode; the page tells the user when this happens. A first uncached trip still needs one
provider to respond, and its selections remain available for retrying. If a dense
short-trip graph still exceeds the cap, the backend automatically retries it using
the same compact coverage strategy.
